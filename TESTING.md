...
### Issue: Files created with root permissions

**Symptom**: `test-out/`, `happy_log.txt`, and other build artifacts are owned by root and you can't delete them.

**Cause**: Docker container was run as root user instead of your user.
...
**Prevention**: The `test-docker-setup.sh` script automatically sets UID/GID. When running `docker-compose` manually, always export UID and GID first.
