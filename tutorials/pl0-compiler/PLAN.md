...
Since this plan was written, rtk gained named constructor labels
(`Expr = Add: Expr '+' Term | ,Term ;`). The first task that touches
`pl0.pg` should label its alternatives and replace the tutorial's
remaining positional `Ctr__*` matches (the Block/Program shells, leaf
unwrapping, the `begin` list) with the labeled names — after that, the
plain-constructor caveats in the task blobs below read better than they
sound.
