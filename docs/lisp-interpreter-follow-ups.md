...
surfaced (valuable independently of it); L3 is blocked on upstream task 8a;
L4–L8 grow the interpreter toward Norvig's fuller `lispy.py`; L9–L11 are
...
## L3 — Adopt named constructors when task 8a lands (blocked)

`qq-grammar-rewrites-plan.md` task 8a adds per-alternative constructor
naming to the grammar language. When it lands, name the alternatives in
scheme.pg (`Num`, `Flt`, `Str`, `TrueL`, `FalseL`, `Sym`, `List`) and
delete the hand-written pattern-synonym block in `Main.hs` (or reduce it
to position-erasing wrappers if 8a names alone don't erase `RtkPos`).
